# ./deepNerRouter1.0.py
# Single-file PoC version combining requirement + question extraction
# Original requirement logic untouched – question branch added as parallel concern
...
Should I, given CDP is disabled, not be able to see Cisco devices?
Should I, given link flap, try switching on LLDP?
...
# core_text = """
# I am working on the Caledonia trains and looking to set the AP configuration, 
# I am missing the mqtt IP address can you find it for me please?
# """

core_text = """
Data is made of structures, doors are a structure, locks are a structure, what is the correct struture for the Access Point lldp request to obtain the switch its connected to?
"""
# ────────────────────────────────────────────────
...
def classify_scope(fact):
    """Classifies scope into architectural levels."""
...
def classify_question_type(sent_text: str) -> str:
    """Classify coarse question intent type."""
...
# ────────────────────────────────────────────────
# VII. NON-NLP EXTRACTION
# ────────────────────────────────────────────────

def extract_dependencies(text):
    """Scans for Requires: / Dependencies: style declarations."""
    dependency_facts = []
...
# ────────────────────────────────────────────────
# VIII. QUESTION EXTRACTION HELPER
# ────────────────────────────────────────────────

def extract_question_fact(sent) -> list:
    """Extract structured question fact from sentence if it looks like a question."""
    sent_text = sent.text.strip()
    if not (sent_text.endswith("?") or sent[0].lemma_ in ["how","why","what","where","when","who","should","can","could"]):
        return []

    qtype = classify_question_type(sent_text)

    # Find main verb (prefer ROOT)
    ...
    for token in sent:
        if token.dep_ in ("advmod", "prep", "advcl", "acomp", "obl"):
            ...
            if len(subtree.split()) > 1:  # avoid very short fragments
    ...
    fact = {
        ...
        "object": " ".join(filter(None, object_parts)).strip(),
...
def systematic_fact_extraction(text):
    ...
    for sent in doc.sents:
        ...
        # Skip weak introductory sentences (original logic)
        ...
        for token in sent:
            if token.dep_ in ("prep", "advcl", "advmod", "appos"):
                if token.head.pos_ != "VERB" or token.dep_ in ("advcl", "advmod"):
                    ...
                    all_sentence_constraints.append(subtree_text)
        ...
        if is_question_sentence:
            ...
            continue   # skip requirement processing for this sentence

        # ── ORIGINAL REQUIREMENT EXTRACTION (unchanged) ────
...
# ────────────────────────────────────────────────
# X. MAIN EXECUTION & OUTPUT FORMATTING
# ────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    print("\n--- 🤖 DEEP NER ROUTER 1.0 (REQUIREMENTS + QUESTIONS) ---", file=sys.stderr)
    print(f"Model loaded → extraction completed in {duration_sec:.3f} s ({duration_ms:.0f} ms)", file=sys.stderr)
